import string
import itertools
inListNumsAsStringArray =  [['SEND', 'MORE']] 
inResultsArray = [ 'MONEY']
...
def solveCryptarithmetic(inListNumsAsString, inResultStr, inPossibleNumsAsStr):
    nonZeroLetters = []
    strFromStrList = ''
...
for i in range(len(inResultsArray)):
    solveCryptarithmetic(inListNumsAsStringArray[i], inResultsArray[i], inPossibleNumsAsStr)
